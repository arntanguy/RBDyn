...
rbd::RBDynFromYAML::RBDynFromYAML(
	...
	verbose_(false),
	transform_inertia_(transform_inertia),
	link_idx_(1),
	joint_idx_(1),
	filtered_links_(filtered_links),
	with_virtual_links_(with_virtual_links),
	spherical_suffix_(spherical_suffix)
{
	joint_types_ = std::map<std::string, rbd::Joint::Type>{
		{"revolute",rbd::Joint::Rev},
		{"continuous",rbd::Joint::Rev},
		{"prismatic",rbd::Joint::Prism},
		{"spherical",rbd::Joint::Spherical},
		{"free",rbd::Joint::Free},
...
void rbd::RBDynFromYAML::parseFrame(const YAML::Node& frame, const std::string& name, Eigen::Vector3d& xyz, Eigen::Vector3d& rpy) {
	xyz.setZero();
	rpy.setZero();
	if(frame) {
...
void rbd::RBDynFromYAML::parseInertia(const YAML::Node& inertia, Eigen::Matrix3d& inertia_mat) {
	...
	if(inertia) {
		inertia_mat = makeInertia(
		    inertia["Ixx"].as<double>(1.),
		    inertia["Iyy"].as<double>(1.),
		    inertia["Izz"].as<double>(1.),
		    inertia["Iyz"].as<double>(0.),
		    inertia["Ixz"].as<double>(0.),
		    inertia["Ixy"].as<double>(0.));
	}
}

void rbd::RBDynFromYAML::parseInertial(const YAML::Node& inertial, const std::string& name, double& mass, Eigen::Vector3d& xyz, Eigen::Vector3d& rpy, Eigen::Matrix3d& inertia) {
	mass  = 1.;
	xyz.setZero();
	rpy.setZero();
	inertia.setIdentity();
...
void rbd::RBDynFromYAML::parseVisuals(const YAML::Node& visuals, std::vector<rbd::Visual>& data) {
	Eigen::Vector3d xyz;
	Eigen::Vector3d rpy;
...
void rbd::RBDynFromYAML::parseLink(YAML::Node link) {
	...
	if(res.visual[name].empty()) {
		res.visual.erase(name);
	}

	parseVisuals(link["collision"], res.collision[name]);
	if(res.collision[name].empty()) {
		res.collision.erase(name);
	}
	...
	if(verbose_) {
		...
		for(const auto& visual : res.visual[name]) {
			std::cout << "\tvisual:\n";
			std::cout << "\t\tname: " << visual.name << '\n';
			std::cout << "\t\torigin: " << visual.origin.translation().transpose() << ", " << visual.origin.rotation().eulerAngles(0,1,2).transpose() << '\n';
			int status = 0;
...
bool rbd::RBDynFromYAML::parseJointType(const YAML::Node& type, const std::string& name, rbd::Joint::Type& joint_type, std::string& type_name) {
	if(type) {
		type_name = type.as<std::string>();

		if(type_name == "floating") {
			bool hasSphericalSuffix = name.length() >= spherical_suffix_.length()
		...
		try {
			...
			throw std::runtime_error("YAML: unkown joint type " + type_name + " (" + name + "). Possible values are: " + possibilities);
		}
...
void rbd::RBDynFromYAML::parseJointLimits(const YAML::Node& limits, const std::string& name, const rbd::Joint& joint, bool is_continuous) {
	...
	auto check_limit =
	[&joint](const std::string & name, const std::vector<double> &limit)
	{
		if(limit.size() != static_cast<size_t>(joint.dof()))
		{
			std::cerr << "YAML: joint " << name << " limit for " << joint.name() << ": size missmatch, expected: " << joint.dof() << ", got: " << limit.size() << std::endl;
		}
	};
...
void rbd::RBDynFromYAML::parseJoint(YAML::Node joint) {
	...
	if(std::find(filtered_links_.begin(), filtered_links_.end(), child) != filtered_links_.end() or
	...
	if(verbose_) {
		...
		std::cout << "\tlimits:\n";
		std::cout << "\t\tlower: [";
		std::copy (res.limits.lower[name].begin(), res.limits.lower[name].end(), std::ostream_iterator<double>(std::cout, ", "));
		std::cout << "\b\b]\n";
		std::cout << "\t\tupper: [";
		std::copy (res.limits.upper[name].begin(), res.limits.upper[name].end(), std::ostream_iterator<double>(std::cout, ", "));
		std::cout << "\b\b]\n";
		std::cout << "\t\tvelocity: [";
		std::copy (res.limits.velocity[name].begin(), res.limits.velocity[name].end(), std::ostream_iterator<double>(std::cout, ", "));
		std::cout << "\b\b]\n";
		std::cout << "\t\ttorque: [";
		std::copy (res.limits.torque[name].begin(), res.limits.torque[name].end(), std::ostream_iterator<double>(std::cout, ", "));
		std::cout << "\b\b]\n";
	}
