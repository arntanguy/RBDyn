...
std::string parseMultiBodyGraphFromURDF(ParserResult& res, const std::string & content, const std::vector<std::string> & filteredLinksIn, bool transformInertia, const std::string & baseLinkIn, bool withVirtualLinks, const std::string sphericalSuffix)
{
	...
	for(tinyxml2::XMLElement * linkDom : links)
	{
		...
		for (tinyxml2::XMLElement *child = linkDom->FirstChildElement("collision");
					...
					else
					{
						...
						else
						{
							...
							if (superellipsoidDom)
							{
								...
								std::cerr << "Warning: only mesh geometry is supported, collision element has been ignored" << std::endl;
						}
					}
				}
				const char* name = child->Attribute("name");
...
ParserResult rbdyn_from_urdf(const std::string & content, bool fixed, const std::vector<std::string> & filteredLinksIn, bool transformInertia, const std::string & baseLinkIn, bool withVirtualLinks, const std::string sphericalSuffix)
{
	ParserResult res;
...
{
	if(input_type == rbd::ParserInput::File) {
		std::ifstream file(input);
		std::string content((std::istreambuf_iterator<char>(file)),
		                    std::istreambuf_iterator<char>());
		res = rbdyn_from_urdf(content,
		                      fixed,
		                      filteredLinksIn,
		                      transformInertia,
		                      baseLinkIn,
